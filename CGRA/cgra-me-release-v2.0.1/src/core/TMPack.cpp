...
TMPack::TMPack(
    ...
    std::vector<std::vector<std::vector<MRRGNodeDesc>>> l_routing_nodes_3d (l_rows, std::vector<std::vector<MRRGNodeDesc>>(l_cols, std::vector<MRRGNodeDesc>{} ));
    std::vector<std::vector<std::vector<MRRGNodeDesc>>> l_function_nodes_3d(l_rows, std::vector<std::vector<MRRGNodeDesc>>(l_cols, std::vector<MRRGNodeDesc>{}));
            ...
            for (auto& fanout : node->fanout) {
                if (std::find(l_function_nodes[submodule].begin(), l_function_nodes[submodule].end(), fanout) == l_function_nodes[submodule].end() && 
                    std::find(l_routing_nodes[submodule].begin(), l_routing_nodes[submodule].end(), fanout) == l_routing_nodes[submodule].end()) {
                        ...
                        uint x_loc = node->parent->loc.x_coord;
                        uint y_loc = node->parent->loc.y_coord;
...
bool TMPack::checkConnectedOperations(std::pair<OpGraphOpDesc, OpGraphOpDesc> op_pair, std::map<OpGraphOpDesc, std::vector<MRRGNodeDesc>>& ops_mrrg_nodes,
    ...
    for (auto source_mrrg_node : ops_mrrg_nodes[op_pair.first]) {
        ...
        for (auto& sink_mrrg_node : ops_mrrg_nodes[op_pair.second]) {
            reached_sink_port = isReachable(source_mrrg_node, sink_mrrg_node, submodule, cycles_to_sink);
...
bool TMPack::isReachable(MRRGNodeDesc source, MRRGNodeDesc sink, std::string submodule, Latency cycles) {
    ...
    while (!to_visit.empty() && !found) {
        ...
        for (const auto& fanout : explore_curr->fanout) {
            ...
            if (fanout->bitwidth < sink->bitwidth && fanout->bitwidth < source->bitwidth) continue;
            auto data_curr = data[{explore_curr, cycles_curr}];
            // Avoiding loop back
