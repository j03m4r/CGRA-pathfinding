...
module dp_ram
	...
	localparam bytes = 2**BANK_ADDR_WIDTH;
    ...
    always @(posedge clk_i or negedge rst_ni or posedge rst_CGRA) begin 
	if (~rst_ni) begin
		rdata_b_o = '0;
	end
	if (rst_CGRA) begin
		rdata_a_o = '0;
        	...
        	if (en_a_i) begin
	    		if (en_b_i == 0 || addr_a_int != addr_b_int) begin
	    			/* handle writes */
				if (we_a_i) begin
